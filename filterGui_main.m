...
% 3. Use the "Filter Method" dropdown to select the filter type (LMS, RLS, or FxLMS).
...
function processButtonPushed(btn, fig)
    ...
    playSignalHelper(fig, noisy_signal, fs, 'Playing original signal with added noise...');
    playSignalHelper(fig, cleaned_signal, fs, 'Playing cleaned signal...');
    if strcmp(filter_type, 'LMS')
        playSignalHelper(fig, amplifiedAudio, fs, 'Playing AMPLIFIED and CLEAN SIGNAL cutted from attack impuls...');
    else
        playSignalHelper(fig, amplifiedAudio, fs, 'Playing CLEAN SIGNAL cutted from attack impuls...');
    end
end
...
function playSignal(fig, signalType)
    ...
    switch signalType
        ...
        case 'noisy'
            ...
            message = 'Playing original signal with added noise...';
        case 'amplified'
